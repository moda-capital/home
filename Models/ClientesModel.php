...
class ClientesModel extends Query
{
    ...
    public function registrarDetalle($produto, $precio, $cantidad, $id_pedido)
    {
        $sql = "INSERT INTO detalle_pedidos (producto, precio, cantidad, id_pedido) VALUES (?, ?, ?, ?)";
        $datos = array($produto, $precio, $cantidad, $id_pedido);
        $data = $this->insertar($sql, $datos);
        if ($data > 0) {
            $res = $data;
        } else {
    ...
    public function getProductos($id_cliente) //recibir parametro
    {
