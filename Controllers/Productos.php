...
class Productos extends Controller
{
    ...
    public function listar()
    {
        ...
        for ($i=0; $i < count($data); $i++) { 
            $data[$i]['imagen'] = '<img class="img-thumbnail" src="'.$data[$i]['imagen'].'" alt="'.$data[$i]['nombre'].'"  width="70px">';
...
public function registrar()
    {
        if (isset($_POST['nombre']) && isset($_POST['precio'])) {
            ...
            if (empty($nombre) || empty($precio) || empty($cantidad)) {
                ...
                if (empty($id)) {
                     $data = $this->model->registrar($nombre, $descripcion, $precio, $cantidad, $destino, $categoria, $color);
                        if ($data > 0) {
                            if (!empty($imagen['name'])) {
                    ...
                    $data = $this->model->modificar($nombre, $descripcion, $precio, $cantidad, $destino, $categoria, $color, $id);
                    if ($data == 1) {
                        if (!empty($imagen['name'])) {
                            move_uploaded_file($tmp_name, $destino);
                        }
    ...
    //eliminar productos
    public function delete($editPro) 
    {
        if (is_numeric($editPro)) {
            $data = $this->model->eliminar($editPro);
                if ($data == 1) {
